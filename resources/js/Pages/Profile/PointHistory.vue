...
usePage().props.permissions.forEach(function(val,key){
    if(val.module.name == 'Religion' && val.role_id == user.role_id){
...
<template>
    <AppLayout title="ধর্ম">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <SucMesgShow :message="$page.props.flash.suc_msg"></SucMesgShow>
                        <div class="bg-white flex justify-between p-4">
                            <h2 class="float-left text-3xl font-extrabold">সর্বোমোটঃ {{en2bn( Number(data.total_sp) + Number(data.points)) }}</h2>
                            <h2 class="float-left text-3xl font-extrabold">ব্যায়ঃ {{ en2bn(Number(data.total_sp))}}</h2>
                            <h2 class="float-left text-3xl font-extrabold">অবশিষ্টঃ {{ en2bn(Number(data.points)) }}</h2>
                        </div>
                        <hr class="mb-1">
                        <table class="w-full text-md text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="(value, key) in mdata.point_history" :key="key"
                                    ...
                                    <td class="px-6 py-4 flex item-center justify-center" v-if="ncheck('edit') || ncheck('delete')">
                                        ...
                                        <button @click="deleting(value.id,'Religion')"  v-if="ncheck('delete')" class="font-medium ml-2 text-red-600 dark:text-blue-500 hover:underline">
